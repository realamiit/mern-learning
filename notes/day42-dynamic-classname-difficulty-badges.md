...
6. **Silent push failure (major debugging incident):** Code local mein sahi tha, lekin `git push` pehli baar silently fail/skip ho gaya — GitHub aur Render dono purani CSS file serve karte rahe (badges plain text dikh rahe the, koi color nahi). Debug karne ka tarika: Network tab mein actual deployed CSS file open karke `.easy` search kiya — nahi mila, jisse confirm hua ki GitHub pe bhi class missing thi. Root cause: commit/push properly complete nahi hua tha. **Lesson: push ke baad hamesha `git status` se confirm karo ki "nothing to commit, working tree clean" dikh raha hai, aur GitHub pe file directly check karo agar UI mein changes na dikhein.**
